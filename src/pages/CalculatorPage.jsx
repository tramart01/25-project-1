...
import {
  berekenTotaal,
  berekenVoedingsWaardes,
  berekenenVoedingswaardesTabel,
} from "../logic/berekenenVoedingswaardes";
...
export default function CalculatorPage(props) {
  ...
  const [voedingsmiddelenLijst, setVoedingsmiddelenLijst] = useState({
    enteraal: [],
    parenteraal: [],
  });

  //Lijst met de totale voedingswaarde voor de enterale en parenterale voedingsmiddelen + het totale aantal voedingsmiddelen
  const [voedingswaardeLijst, setVoedingswaardeLijst] = useState({
    totaal: {},
    enteraal: {},
    parenteraal: {},
  });

  const [voedingswaardeTabel, setVoedingswaardeTabel] = useState({
    totaal: {},
    enteraal: {},
    parenteraal: {},
  });
  ...
  useEffect(() => {
    ...
    for (let voedingsmiddel in voedingsmiddelenLijst) {
      berekendeWaarden[voedingsmiddel] = berekenVoedingsWaardes(
        voedingsmiddelenLijst[voedingsmiddel],
      );
    }
  ...
  useEffect(() => {
    ...
    for (let voedingsmiddel in voedingswaardeLijst) {
      berekendeWaarden[voedingsmiddel] = berekenenVoedingswaardesTabel(
        voedingswaardeLijst[voedingsmiddel],
        gewicht,
      );
    }
  ...
  useEffect(() => {
    ...
    if (
      voedingswaardeLijst.totaal.hoeveelheid !== null &&
      (voedingswaardeLijst.totaal.hoeveelheid > ABSOULTEWAARDEVOCHT ||
        voedingswaardeLijst.totaal.hoeveelheid / gewicht > grens)
    ) {
  ...
  return (
    <div className="bereken-pagina-container">
      <AlertModal
        ...
        onConfirm={() => navigate("/")}
      >
        Er zijn gegevens ingevuld, als je deze pagina verlaat zullen deze niet
        opgeslagen worden. Wil je doorgaan?
      </AlertModal>

      <AlertModal
        open={openAlertGrens}
        onClose={setOpenAlertGrens}
        onConfirm={null}
      >
        Er is een grenswaarde bereikt. Let op dat je de patiënt niet teveel
        vocht geeft
      </AlertModal>
      ...
      <div className="header">
        <HomeIcon
          className="home-icon hover-icon"
          fontSize="large"
          onClick={handleOnHomeClicked}
        />
        <Typography
          className="titels"
          level="h1"
      ...
      <div className="gegevens-invoer">
        <UserInput
          naamClass="invoer-datum"
          type="date"
          label="Geboortedatum"
          placeholder=""
          value={datum}
          changeValue={setDatum}
          onDefocus={berekenLeeftijd}
        />
        <UserInput
          naamClass="invoer-gewicht"
          type="text"
          label="Gewicht"
          placeholder="18 kg"
          value={gewichtInput}
          changeValue={setGewichtInput}
          onDefocus={handleGewichtChange}
        />
      </div>
      ...
      <div className="bewaren-button-container">
        <Button
          className="bewaren-button button"
          onClick={handlePatientOpslaan}
        >
          Bewaren
        </Button>
      </div>
  ...
  function berekenLeeftijd(datum) {
    ...
    if (
      huidigeDatum.getMonth() < geboorteDatum.getMonth() ||
      (huidigeDatum.getMonth() === geboorteDatum.getMonth() &&
        huidigeDatum.getDate() < geboorteDatum.getDate())
    ) {
      berekendeLeeftijd--;
    }
            ...
            {tableRows.map((row, index) => {
              return (
                <tr key={index} className={row.bold ? "tabel-dikgedrukt" : ""}>
                  <td className={row.bold ? "tabel-dikgedrukt" : ""}>
                    {row.label}
                  </td>
                  <td>{row.subLabel}</td>
  ...
  function handlePatientOpslaan() {
    ...
    setOpgeslagenObject({
      datum,
      gewicht,
      voedingsLijst: voedingsmiddelenLijst,
    });
    storePatientData(datum, {
      datum,
      gewicht,
      voedingsLijst: voedingsmiddelenLijst,
    });
  ...
  function handleOnHomeClicked() {
    ...
    if (
      gewicht === 0 &&
      leeftijd === 0 &&
      voedingsmiddelenLijst.enteraal.length === 0 &&
      voedingsmiddelenLijst.parenteraal.length == 0
    ) {
      navigate("/");
    } else if (
      JSON.stringify(opgeslagenObject) === JSON.stringify(objectTest)
    ) {
      navigate("/");
    } else {
      setOpenAlert(true);
